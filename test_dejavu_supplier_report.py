...
def test_generate_supplier_report_with_dejavu():
    ...
    with app.app_context():
        ...
        # Test all three PDF generation methods
        
        # 1. Default method (WeasyPrint)
        print("Testing default WeasyPrint method...")
        ...
        output_file = f"test_report_weasyprint_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        with open(output_file, "wb") as f:
            f.write(pdf_content)
        print(f"WeasyPrint PDF saved to {output_file}")
        
        # 2. FPDF method
        print("Testing FPDF method...")
        pdf_content, filename = generate_custom_supplier_report(
            quotation, 
            suppliers, 
            ["description", "scientific_name", "pot_size", "height"],
        ...
        output_file = f"test_report_fpdf_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        with open(output_file, "wb") as f:
            f.write(pdf_content)
        print(f"FPDF PDF saved to {output_file}")
        
        # 3. DejaVu method
        print("Testing DejaVu method...")
        pdf_content, filename = generate_custom_supplier_report(
            quotation, 
            suppliers, 
            ["description", "scientific_name", "pot_size", "height"],
        ...
        output_file = f"test_report_dejavu_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        with open(output_file, "wb") as f:
            f.write(pdf_content)
...
if __name__ == "__main__":
    test_generate_supplier_report_with_dejavu()