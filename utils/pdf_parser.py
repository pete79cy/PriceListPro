import re
import logging
import PyPDF2
from datetime import datetime

def extract_text_from_pdf(pdf_path):
    """
    Extract text content from a PDF file.
    
    Args:
        pdf_path (str): Path to the PDF file
        
    Returns:
        str: Extracted text content
    """
    logging.debug(f"Extracting text from PDF: {pdf_path}")
    
    try:
        text = ""
        with open(pdf_path, 'rb') as file:
            pdf_reader = PyPDF2.PdfReader(file)
            for page_num in range(len(pdf_reader.pages)):
                text += pdf_reader.pages[page_num].extract_text()
        return text
    except Exception as e:
        logging.error(f"Error extracting text from PDF: {str(e)}")
        raise

def extract_invoice_data(text):
    """
    Extract invoice data from the PDF text using regex and NLP techniques.
    
    Args:
        text (str): Text content from the PDF
        
    Returns:
        dict: Extracted invoice data including invoice number, date, items, and totals
    """
    logging.debug("Extracting invoice data from text")
    
    invoice_data = {
        'invoice_number': None,
        'invoice_date': None,
        'total_amount': None,
        'items': []
    }
    
    # Extract invoice number
    invoice_number_match = re.search(r'(?:Invoice|INV|Invoice Number|Invoice #)[:\s]+([A-Za-z0-9\-]+)', text, re.IGNORECASE)
    if invoice_number_match:
        invoice_data['invoice_number'] = invoice_number_match.group(1).strip()
    
    # Extract invoice date
    date_patterns = [
        r'(?:Invoice|Bill) Date[:\s]+(\d{1,2}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4})',
        r'(?:Date|Dated)[:\s]+(\d{1,2}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4})',
        r'(\d{1,2}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4})'  # Generic date pattern
    ]
    
    for pattern in date_patterns:
        date_match = re.search(pattern, text, re.IGNORECASE)
        if date_match:
            date_str = date_match.group(1).strip()
            # Try to parse the date in various formats
            for date_format in ['%d/%m/%Y', '%m/%d/%Y', '%d-%m-%Y', '%m-%d-%Y', '%d.%m.%Y', '%m.%d.%Y']:
                try:
                    date_obj = datetime.strptime(date_str, date_format)
                    invoice_data['invoice_date'] = date_obj.strftime('%Y-%m-%d')
                    break
                except ValueError:
                    continue
            if invoice_data['invoice_date']:
                break
    
    # Extract total amount
    amount_patterns = [
        r'(?:Total|Amount Due|Grand Total|Sum)[:\s][$ۣ]?\s*(\d+[.,]\d+)',
        r'[$ۣ]\s*(\d+[.,]\d+)'  # Generic currency pattern
    ]
    
    for pattern in amount_patterns:
        amount_match = re.search(pattern, text, re.IGNORECASE)
        if amount_match:
            amount_str = amount_match.group(1).strip().replace(',', '.')
            try:
                invoice_data['total_amount'] = float(amount_str)
                break
            except ValueError:
                continue
    
    # Extract line items using table detection heuristics
    # This is a simplified approach; real implementation would be more complex
    lines = text.split('\n')
    item_mode = False
    current_item = {}
    
    for line in lines:
        # Skip empty lines
        if not line.strip():
            continue
        
        # Look for table headers that might indicate the start of item listing
        if re.search(r'(item|description|product|service|qty|quantity|price|amount|total)', line, re.IGNORECASE) and not item_mode:
            item_mode = True
            continue
        
        if item_mode:
            # If line contains price-like pattern, it might be an item line
            price_match = re.search(r'(\d+[.,]\d+)', line)
            if price_match:
                # Try to extract item details
                description_match = re.search(r'^(.*?)\s+\d', line)
                if description_match:
                    description = description_match.group(1).strip()
                    price = float(price_match.group(1).replace(',', '.'))
                    
                    # Extract quantity if present
                    qty_match = re.search(r'(\d+)\s*(?:x|pcs|ea)', line, re.IGNORECASE)
                    quantity = float(qty_match.group(1)) if qty_match else 1.0
                    
                    # Add item to the list
                    invoice_data['items'].append({
                        'description': description,
                        'quantity': quantity,
                        'price': price,
                        'total': quantity * price
                    })
            
            # If we encounter a line that might indicate the end of items section
            if re.search(r'(subtotal|tax|vat|total|balance)', line, re.IGNORECASE):
                item_mode = False
    
    logging.debug(f"Extracted invoice data: {invoice_data}")
    return invoice_data
