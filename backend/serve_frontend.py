...
# Define routes on the blueprint first, before it gets registered
@frontend_bp.route('/', defaults={'path': ''})
@frontend_bp.route('/<path:path>')
def serve_frontend(path):
    """Serve the built React frontend"""
    # Get the app from current_app
    app = current_app
    frontend_dir = os.path.join(os.path.dirname(app.root_path), 'frontend', 'dist')
...
def register_frontend_routes(app):
    ...
    # Store the frontend directory path in the app config for access elsewhere
    app.config['FRONTEND_DIR'] = os.path.join(os.path.dirname(app.root_path), 'frontend', 'dist')
